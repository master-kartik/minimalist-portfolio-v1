...
const MainCard = ({inverted,title, desc, img, classNames} : MainCardProps) => {
  ...
  return (
    <div className={` px-6 py-7 md:py-0 md:px-0 md:flex ${inverted ? 'md:flex-row-reverse':'md:flex'} items-center justify-center`} >
        ...
        <div className={`w-full text-neutral-700 ${inverted ? 'text-left':'md:text-right'}  mt-2`}>
            ...
            <div className='md:leading-5 whitespace-pre-wrap'>{desc || 'Interactive, story-telling education tool (in progress)'}</div>
        </div>
        </div>
        <div className={`w-full mt-6 md:w-[40%] md:px-4 z-50 ${classNames}   ${inverted ? 'md:pl-10':'md:pr-8'}`}><Image  alt='image' src={img || 'https://cdn.prod.website-files.com/635dc1a15dee791d705a7eaf/669dc853cd19642dd9ba4922_visavid-ezgif.com-video-to-gif-converter.gif'} width={500} height={500}></Image></div>
    </div>
  )
}
